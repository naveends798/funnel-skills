...
For each stage, use the `Agent` tool with `subagent_type: general-purpose` (or the named subagent if registered). Prompt the agent with: "Read `${CLAUDE_PLUGIN_ROOT}/skills/<skill>/SKILL.md` and run the skill for slug `<slug>`."
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/funnel-orchestrator/scripts/render-dashboard.mjs" <slug>
