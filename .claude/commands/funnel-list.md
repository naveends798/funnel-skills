...
1. **Find all client output folders** in the current working directory:
   ...
   If empty or no `output/`, also check the funnel-skills home for any leftover examples:
   ```bash
   if [ -d "${FUNNEL_SKILLS_HOME:-$HOME/.funnel-skills}/output" ]; then
     ls -1 "${FUNNEL_SKILLS_HOME:-$HOME/.funnel-skills}/output" | grep -v _inbox | sort
   fi
   ```
