...
2. **Run tiered research** via Bash:
   ```bash
   node lib/research-stack.mjs "<niche> target audience pain points" --intent=pain_points --depth=deep
   node lib/research-stack.mjs "<niche> top competitors offers pricing" --intent=competitor --depth=standard
   node lib/research-stack.mjs "<niche> best practices 2026" --intent=general
   ```
...
3. **Read references**: `.claude/skills/market-intelligence/references/awareness-levels.md` for the Schwartz framework definitions.

4. **Synthesize** the research into the schema in `.claude/skills/funnel-orchestrator/references/output-schema.md` (section: 01-market.json).
